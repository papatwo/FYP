...
for l = 1, 18 do --remove layer from bottom to top: output
  vgg:remove()                                                             --|
end 
...
local gradLoss = torch.zeros(vgg:forward(img):size())
local filteridx = 2
gradLoss[filteridx] = 1
print(gradLoss)
for i = 1, 2 do
-- eg. remore 18 layers, output from conv layer 5
local out = vgg:forward(img) --size 256x26x41
local loss = torch.mean(out[filteridx])
-- compute the gradient of the input picture wrt this loss
local grads = vgg:backward(img, gradLoss) 

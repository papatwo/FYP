...
for l = 1, 8  do
  vgg:remove() -- remove from the layer closest to output layer
end

--local img= torch.rand(3,224,224):uniform()
local img = image.load('golden.jpg') -- load the image
--local img = image.load('circle.png')


--- realise a basic gradient asent function 
--- by using jitter and gradient ascent steps normalisation
function make_step(net, img, clip, step_size, jitter)
    ...
    local dst, g
    dst = net:forward(img) -- feedforward to the last layer of reduced net
    g = net:updateGradInput(img,dst) -- get gradient

    -- apply normalized ascent step to the input image
    img:add(g:mul(step_size/torch.abs(g):mean()))


    img = image.translate(img,-ox,-oy) -- image jitter unshift

    if clip then -- if no clamp on the image shows gray background like feature viz
...
---  implement an ascent through different scales. 
---- call these scales “octaves”.
function deepdream(net, base_img, iter_n, octave_n, octave_scale, end_layer, clip, visualize)
    --- # default settings:
    --- original img iterates 15 times, rescale to 1.4 times and iterate again
    --- carry out for 4 rounds
    local iter_n = iter_n or 15 --- iteration times
    local octave_n = octave_n or 4 --- rescale times 尺度缩放次数
    local octave_scale = octave_scale or 1.4 --- rescale ratio 尺度缩放比例
    ...
    local octaves = {}
    --- prepare first round with the original size img
    octaves[octave_n] = torch.add(base_img, -Normalization.mean):div(Normalization.std)
    local _,h,w = unpack(base_img:size():totable())
    --- loop to prepare the next round with width and length shrink by a factor of 1.4
